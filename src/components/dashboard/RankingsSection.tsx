...
function Avatar({
  ...
  if (url) {
    return (
      ...
      <img src={url} alt={name} className={`${cls} rounded-full object-cover ring-2 ring-primary/40`} />
    );
  }
  return (
    <div className={`${cls} rounded-full bg-primary/10 flex items-center justify-center font-semibold text-primary ring-2 ring-primary/20`}>
...
function PodiumCard({
  ...
  return (
    ...
    >
      ...
      <Avatar name={entry.name} url={entry.avatarUrl} size={isFirst ? "lg" : "md"} />
      <p className={`font-semibold truncate max-w-full text-center ${isFirst ? "text-base" : "text-sm"}`}>
        {entry.name}
      </p>
      <p className={`font-bold tabular-nums text-primary ${isFirst ? "text-base" : "text-sm"}`}>
        {formatCurrency(entry.cashRealized)}
      </p>
      <p className="text-xs text-muted-foreground">{entry.podiumSub}</p>
      <span className={`text-sm font-bold rounded-md px-3 py-1 ${PLACE_BADGE[rank] ?? ""}`}>
        {rank + 1}
      </span>
    </button>
  );
...
function RankingBlock({
  ...
  const podium = PODIUM_ORDER.map((i) => entries[i]).filter(Boolean) as RankEntry[];
  const podiumCols =
    entries.length === 1 ? "grid-cols-1 max-w-xs mx-auto" :
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/30">
              <th className="text-left px-3 py-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground w-8">#</th>
              <th className="text-left px-3 py-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Nome</th>
              <th className="text-right px-3 py-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">{midColumnLabel}</th>
              <th className="text-right px-3 py-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Caixa</th>
            </tr>
          </thead>
          <tbody>
            {entries.map((entry, idx) => (
              <tr
                ...
                onClick={() => onSelectUser?.(entry.userId)}
              >
                <td className="px-3 py-2 text-muted-foreground text-xs">{medalOrRank(idx)}</td>
                <td className="px-3 py-2 font-medium">{entry.name}</td>
                <td className="px-3 py-2 text-right text-muted-foreground">{entry.midValue}</td>
                <td className={`px-3 py-2 text-right font-semibold tabular-nums ${pctColor(entry.cashRealized, entry.cashGoal)}`}>
                  {formatCurrency(entry.cashRealized)}
                </td>
              </tr>
            ))}
