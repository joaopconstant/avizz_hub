...
export function ReportForm({
  ...
  const {
    ...
    defaultValues: {
      work_location: (existingReport?.work_location as FormValues["work_location"]) ?? "office",
      ...
      <div>
        ...
        {existingReport && (
          <p className="text-xs text-muted-foreground mt-0.5">
            {isReadOnly ? "Relatório preenchido" : "Editando relatório existente"}
          </p>
        )}
      </div>
          ...
          {isSdr ? (
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                <NumberField label="Ligações Realizadas" name="calls_total" register={register} readOnly={isReadOnly} value={existingReport?.calls_total} />
                <NumberField label="Ligações Atendidas" name="calls_answered" register={register} readOnly={isReadOnly} value={existingReport?.calls_answered} />
                <NumberField label="Reuniões Agendadas" name="meetings_scheduled" register={register} readOnly={isReadOnly} value={existingReport?.meetings_scheduled} />
                <NumberField label="Reuniões Realizadas" name="meetings_held" register={register} readOnly={isReadOnly} value={existingReport?.meetings_held} />
                <NumberField label="Atividades CRM" name="crm_activities" register={register} readOnly={isReadOnly} value={existingReport?.crm_activities} />
                <NumberField label="Conversas no Bot" name="bot_conversations" register={register} readOnly={isReadOnly} value={existingReport?.bot_conversations} />
                <NumberField label="Reagendamentos" name="reschedulings" register={register} readOnly={isReadOnly} value={existingReport?.reschedulings} />
              </div>
            </>
          ) : (
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                <NumberField label="Ligações Realizadas" name="calls_done" register={register} readOnly={isReadOnly} value={existingReport?.calls_done} />
                <NumberField label="No-shows" name="closer_no_shows" register={register} readOnly={isReadOnly} value={existingReport?.closer_no_shows} />
                <NumberField label="Desqualificados" name="disqualified" register={register} readOnly={isReadOnly} value={existingReport?.disqualified} />
              </div>
              {/* CRM atualizado */}
              {isReadOnly ? (
                <div className="space-y-1">
                  <Label className="text-xs text-muted-foreground">CRM Atualizado</Label>
                ...
                <div className="flex items-center gap-2">
                  ...
                  />
                  <Label htmlFor="crm_updated" className="text-sm cursor-pointer">
                    CRM atualizado
                  </Label>
                </div>
              )}
